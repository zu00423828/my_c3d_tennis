...
print(len(validation_video_indices))
with tf.Graph().as_default():
    ...
    with tf.Session(config=config) as sess:
        ...
        for epoch in range(EPOCH_NUM):
            ...
            '''print('Epoch %d: Average loss is: %.5f; Average accuracy is: '%(epoch+1, loss_epoch / (len(train_video_indices) // BATCH_SIZE),accuracy_epoch / (len(train_video_indices) // BATCH_SIZE)))'''
            accuracy_epoch = 0
            loss_epoch = 0
            batch_index = 0
            ...
            print('Validation loss is %.5f; Accuracy is %.5f'%(loss_epoch / (len(validation_video_indices) // BATCH_SIZE),accuracy_epoch /(len(validation_video_indices) // BATCH_SIZE)))
